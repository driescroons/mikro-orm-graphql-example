...
export class Migration20200809143205 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "publisher" ("id" uuid not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) not null, "name" varchar(255) not null, "type" text check ("type" in (\'local\', \'global\')) not null);');
    this.addSql('alter table "publisher" add constraint "publisher_pkey" primary key ("id");');

    this.addSql('create table "author" ("id" uuid not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) not null, "name" varchar(255) not null, "email" varchar(255) not null, "terms_accepted" jsonb not null, "born" timestamptz(0) null, "favourite_book_id" uuid null);');
    ...
    this.addSql('create table "book" ("id" uuid not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) not null, "title" varchar(255) not null, "author_id" uuid not null, "publisher_id" uuid null);');
    this.addSql('alter table "book" add constraint "book_pkey" primary key ("id");');

    this.addSql('create table "book_tags" ("book_id" uuid not null, "tag_id" uuid not null);');
